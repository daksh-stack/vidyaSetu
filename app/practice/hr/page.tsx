...
import { Mic, PlayCircle, StopCircle, User2 } from "lucide-react";
import { useState } from "react";

const questions = [
    { id: 1, text: "Tell me about a challenging project you worked on." },
    { id: 2, text: "Why do you want to work for our company?" },
    { id: 3, text: "Where do you see yourself in 5 years?" },
    { id: 4, text: "Describe a time you had a conflict with a team member." },
];

export default function HRPracticePage() {
    const [selectedQuestion, setSelectedQuestion] = useState(questions[0]);
    const [isRecording, setIsRecording] = useState(false);
    const [recordings, setRecordings] = useState<{ id: number, questionId: number, duration: string }[]>([]);

    const toggleRecording = () => {
        if (isRecording) {
            // Stop recording
            setRecordings([...recordings, { id: Date.now(), questionId: selectedQuestion.id, duration: "0:45" }]);
        }
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="mb-8 text-center mt-12">
                            ...
                            <h1 className="text-3xl font-bold mb-4">{selectedQuestion.text}</h1>
                        ...
                        <div className="mt-12 border-t border-white/10 pt-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {recordings.filter(r => r.questionId === selectedQuestion.id).map((rec, i) => (
                                ...
                                {recordings.filter(r => r.questionId === selectedQuestion.id).length === 0 && (
